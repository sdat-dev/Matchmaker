...
let getResultData = function(jsonData){
    spin_logo = "https://sdat-dev.github.io/resources/wecliemprep/assets/logos-funding-opportunities/SPIN_logo.png";
    let accordionContent = '';
    console.log(jsonData.length);
    for(var i=0;i<jsonData.length;i++){
        accordionContent = generateAccordianContent(spin_logo,solicitations[jsonData[i].id],"01/23/2023");
    }
    console.log(accordionContent);
    // generateAccordionElem(level, collapseId, headerId, parentId, childId, header, accordionContent)
}

let generateAccordianContent = function(logo_url,data,dueDate){
    content += '<div class="display-flex opportunity-container search-container">';
    content += '<div class="col-xl-2 col-lg-3">';
    content += '<img class="agency-logo" src="'+ logo_url +'"></div>';
    content += '<div class="col-xl-10 col-lg-9">';
    content += '<h4 class="opp-header black-content-header-no-margin">' + data.prog_title + '</h4>';
    content += '<div class="opp-details display-flex">';
    content += '<div class="col-sm-12 col-md-12 col-lg-12 col-xl-6">';
    content += '<i class="fas fa-flag"></i><strong>Agency Name: </strong>' + data.spon_name + '<br><i class="fas fa-dollar-sign"></i>';
    content += '<strong>Estimated Funding: </strong>' + data.total_funding_limit + '<br></div>';
    content += '<div class="col-sm-12 col-md-12 col-lg-12 col-xl-6">';
    content += '<i class="fas fa-calendar-day"></i> <strong>Date: </strong>' + dueDate + '<br></div></div></div><br><br><br><br><br><br><br>';
    content += '<p class="opp-description">' + data.synopsis + '</p><p class="width100" style="padding-left: 15px;">';
    content += '<button type="button" class="details-button" onclick="window.open(\'' + data.programurl +'\',\'_blank\')">View Details</button></p></div>';
    return content;
}
